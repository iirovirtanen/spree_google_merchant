...
google_merchant_gtin       = variant.product.properties.where(name: "GTIN")
google_merchant_condition  = variant.product.properties.where(name: "Condition")
...
xml.tag! "g:condition", google_merchant_condition.name
xml.tag! "g:availability", Spree::Stock::Quantifier.new(variant).total_on_hand > 0 ? 'in stock' : 'out of stock'
xml.tag! "g:gtin", google_merchant_gtin.name if google_merchant_gtin
